        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                    ...
                    action: {
                        withAnimation(.appSpring) {
                            if activeFilters.contains(.liveNow) {
                                activeFilters.remove(.liveNow)
                            } else {
                                activeFilters.insert(.liveNow)
                            }
                        }
                    ...
                    action: {
                        withAnimation(.appSpring) {
                            if activeFilters.contains(.trending) {
                                activeFilters.remove(.trending)
                            } else {
                                activeFilters.insert(.trending)
                            }
                        }
                        HapticEngine.selection()
                    }
                ...
                FilterPill(
                    ...
                    action: {
                        withAnimation(.appSpring) {
                            if activeFilters.contains(.nearMe) {
                                activeFilters.remove(.nearMe)
                            } else {
                ...
                // Denomination pills
                ForEach(denominationOptions.prefix(4), id: \.self) { denom in
                    let denominationFilter = DiscoverViewModel.DiscoverFilter.denomination(denom)
                    FilterPill(
                        label: denom,
                        isActive: activeFilters.contains(denominationFilter),
                        action: {
                            withAnimation(.appSpring) {
                                if activeFilters.contains(denominationFilter) {
                                    activeFilters.remove(denominationFilter)
                                } else {
                                    activeFilters.insert(denominationFilter)
                                }
                            }
                            HapticEngine.selection()
                        }
...
struct FilterPill: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(label)
                .font(.system(size: 13, weight: .semibold))
                .foregroundColor(isActive ? .white : .lcText2)
                .padding(.horizontal, 12)
                .padding(.vertical, 8)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 20)
                        .stroke(isActive ? Color.clear : Color.lcBorder, lineWidth: 1)
                )
        }
    }
}
