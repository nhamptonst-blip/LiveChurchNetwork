...
struct ChurchCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Cover image and info wrapped in NavigationLink
            NavigationLink(destination: ChurchDetailView(church: church)) {
                VStack(alignment: .leading, spacing: 0) {
                        ...
                        if !church.image.isEmpty, let url = URL(string: church.image) {
                            AsyncImage(url: url) { phase in
                                ...
                                case .success(let img):
                                    img.resizable()
                                        .scaledToFill()
                                        .opacity(0.0)  // Hide custom image in cover area
                                default:
                                    EmptyView()
                                }
                            }
                    ...
                    // Content area (non-interactive part)
                    VStack(alignment: .leading, spacing: 8) {
                        // Top spacing for overlapping logo
